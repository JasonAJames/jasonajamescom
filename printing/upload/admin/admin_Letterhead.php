...
if ($password_protect == "off") {
	...
	// Check crypted pass against stored pass. Check random string and pass hashed concatenate against post. Check hashed and crypted agent/language concatenate against post.
		...
		<form action=\"admin_Letterhead.php\" method=\"post\" name=\"form1\" onsubmit=\"return obfuscate()\">
		$p
...
<tr>
<td colspan=\"6\">
<form action=\"admin_Letterhead.php\" method=\"post\" enctype=\"multipart/form-data\">
<div id =\"upload_popup\">
...
$upload_dir = ($_SERVER['DOCUMENT_ROOT'] . "/$fileupload_Letterhead_dir_name");
...
<form action=\"admin_Letterhead.php\" method=\"post\">
$p
...
echo "
<td class=\"uploadlistloc\"><a href=\"http://".$_SERVER['HTTP_HOST']."/$fileupload_Letterhead_dir_name/$file\"><i>http://".$_SERVER['HTTP_HOST']."/$fileupload_Letterhead_dir_name/$file</i></a></td>
</tr>";
}
}
closedir($dir_handle);
...
// Path to file.
$upload_file_path = ($_SERVER['DOCUMENT_ROOT'] . "/$fileupload_Letterhead_dir_name/$file");
// Delete file
...
echo "<script type=\"text/javascript\">
<!--
var URL   = \"admin_Letterhead.php?pg=$pg\"
var speed = $upload_redirect
function reload() {
location = URL
}
...
if (file_exists($upload_file_path)) {
	rename ($upload_file_path, $_SERVER['DOCUMENT_ROOT'] . "/$fileupload_Letterhead_dir_name/$upload_newname") or die ("$l_upload19");
